...
class SinglePage():
	def __init__(self, url, ip, jump_url, header):
		self.url = url
		self.ip = ip
		self.jump_url = jump_url
		self.img_url = url.replace('forum.php', '')
		self.header = header
		self.header['refer'] = header['refer'] + '&filter=sortid&sortid=3&searchsort=1&area=1'
	...
	def getSinglePageContent(self):
		...
		try:
			...
			complete = contentDetail.Complete(titleData, contentData, imageData, qqData, wxData, url, tid)
			return complete
